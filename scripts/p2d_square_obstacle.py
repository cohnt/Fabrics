...
def repeller_task_map(theta):
	...
	for obs_origin, obs_r in zip(obs_origins, obs_rs):
		dx = np.max(np.array([(obs_origin[0] - obs_r) - theta[0], 0, theta[0] - (obs_origin[0] + obs_r)]))
		dy = np.max(np.array([(obs_origin[1] - obs_r) - theta[1], 0, theta[1] - (obs_origin[1] + obs_r)]))
		arr.append(np.linalg.norm(np.array([dx, dy])))
	return np.array(arr)
...
for obs_origin, obs_r in zip(obs_origins, obs_rs):
	obs_element = plt.Rectangle(tuple(obs_origin - np.array([obs_r, obs_r])), 2*obs_r, 2*obs_r, color='black')
